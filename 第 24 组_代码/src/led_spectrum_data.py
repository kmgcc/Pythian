...
# 五个窄带彩色通道的目标峰值波长，每个通道从下载的 Harald Brendel 实测 LED 光谱
# 数据集中匹配峰值最接近的窄带 LED，保证全部通道都有实测光谱支撑。
...
def build_dual_white_from_measured_leds(measured_df: pd.DataFrame) -> pd.DataFrame:
    """根据实测蓝光泵浦和荧光粉 LED 构建 2700K/6500K 白光 LED 通道。"""
...
def fetch_and_build_dual_white_spectrum(
    ...
    """获取实测 LED SPD 数据并保存派生的双色温白光 LED 通道。"""
...
def load_dual_white_spectrum(
    ...
    """返回双色温白光 LED 通道，全部由实测 SPD 数据派生。

    若缓存 CSV 不存在，从实测 LED 数据集重建暖白/冷白通道（按需下载），无合成回退。
    """
    path = Path(path)
    if path.exists():
...
def source_frame(force: bool = False) -> pd.DataFrame:
    """返回 LED 数据来源信息；可选地下载参考 HTML 作为缓存依据。"""
